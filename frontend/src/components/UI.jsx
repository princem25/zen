    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 ${scrolled ? 'glass-panel py-3' : 'py-5'}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-10 flex items-center justify-between">
          <Link to="/" className="font-display text-2xl md:text-3xl font-light tracking-[0.18em] text-charcoal relative z-[1002]">ZENPHIRA</Link>
        ...
        <ul className="flex flex-col items-center gap-10 m-0 p-0">
          ...
          {user ? (
             <li className="relative mt-4">
               <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="block font-body text-[1.6rem] sm:text-[1.4rem] tracking-[0.08em] text-charcoal opacity-100 translate-y-0 transition-all">
                 Logout
               </button>
             </li>
          ) : (
             <li className="relative mt-4 flex flex-col gap-4">
               <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="block font-body text-[1.6rem] sm:text-[1.4rem] tracking-[0.08em] text-charcoal transition-all text-center">
                 Log In
               </Link>
               <Link to="/register" onClick={() => setMobileMenuOpen(false)} className="block font-body text-[1.6rem] sm:text-[1.4rem] tracking-[0.08em] text-charcoal transition-all text-center">
                 Sign Up
               </Link>
             </li>
          )}
      ...
      <div className="flex flex-col md:flex-row items-center w-full gap-12 md:gap-8 z-10">
        <div className="flex-1 max-w-2xl text-center md:text-left">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
            <Link to="/products" className="bg-gradient-to-br from-blush-deep to-[#d4788a] text-white px-8 py-3.5 rounded-full text-sm font-medium tracking-wide shadow-[0_8px_32px_rgba(232,160,176,0.4)] hover:-translate-y-1 hover:shadow-[0_16px_48px_rgba(232,160,176,0.55)] transition-all">
              Shop Collection
            </Link>
